...
public class ArmMath {
  ...
  /**
   * Calculates the maximum arm length that can be achieved at the specified angle without exceeding
   * the specified horizontal measurement.
   *
  ...
  public static double calculateHorizontalLengthConstraint(double angle, double horizontal) {
    ...
    // If the arm is vertical, the horizontal measurement will never be exceeded, so this constraint
    // is not a factor
    if (cos == 0) {
  ...
  /**
   * Calculates the maximum arm length that can be achieved at the specified angle without exceeding
   * the specified vertical measurement.
   *
   * @param angle the arm angle, in degrees.
  ...
  public static double calculateLengthConstraint(double angle) {
    ...
    double horizontal =
        calculateHorizontalLengthConstraint(angle, Constants.Arm.Extension.MAX_HORIZONTAL_LENGTH);
    double vertical =
        calculateVerticalLengthConstraint(angle, Constants.Arm.Extension.MAX_VERTICAL_EXTENSION);
